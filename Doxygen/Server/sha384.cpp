...
/**
 * @brief Циклический сдвиг вправо (ROTR)
 */
...
/**
 * @brief Логический сдвиг вправо (SHR)
 */
...
/**
 * @brief Основная функция преобразования Σ0
 */
...
/**
 * @brief Основная функция преобразования Σ1
 */
...
/**
 * @brief Вспомогательная функция преобразования Γ0
 */
...
/**
 * @brief Вспомогательная функция преобразования Γ1
 */
...
/**
 * @brief Дополнение сообщения до длины кратной 128 байтам
 * 
 * Добавляет:
 * 1. Бит '1' (байт 0x80)
 * 2. Нулевые биты до последних 16 байт
 * 3. 128-битную длину исходного сообщения (big-endian)
 * 
 * @throws std::length_error Если сообщение слишком большое
 */
...
/**
 * @brief Обрабатывает один 1024-битный блок
 * 
 * Алгоритм:
 * 1. Подготовка сообщения (W[0..79])
 * 2. Инициализация рабочих переменных
 * 3. 80 раундов преобразований
 * 4. Обновление состояния хеша
 */
 ...
 * 
 * Этапы:
 * 1. Преобразование строки в байты (UTF-8)
 * 2. Дополнение сообщения
 * 3. Инициализация начального состояния
 * 4. Обработка блоков по 128 байт
 * 5. Формирование результата из первых 384 бит (6 регистров)
 * 
 * @return 96-символьная шестнадцатеричная строка (384 бита)
 */
