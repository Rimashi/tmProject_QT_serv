...
/**
 * @brief Генерирует простое число заданной битности
 * 
 * Гарантирует установку старшего бита для достижения нужного размера.
 * Использует 30 раундов теста Миллера-Рабина для надежности.
 */
...
/**
 * @brief Проверяет, является ли число вероятно простым
 * 
 * @param k определяет точность теста (вероятность ошибки 4^(-k))
 */
...
/**
 * @brief Вычисляет обратный элемент в кольце вычетов
 * 
 * @throws Фактически возвращает 0 при ошибке (с предупреждением)
 */
 ...
 * 4. Выбор открытой экспоненты e (65537 по умолчанию)
 * 5. Проверка gcd(e, φ(n)) == 1
 * 6. Вычисление d = e⁻¹ mod φ(n)
 * 
 * @note Использует криптографически безопасный ГСЧ
 */
...
/**
 * @brief Шифрует строку с автоматическим Base64 кодированием
 */
 ...
 * 
 * Алгоритм:
 * 1. Разбор открытого ключа
 * 2. Определение размера блока: (bitlen(n)/8) - 1
 * 3. Разбиение данных на блоки
 * 4. Шифрование каждого блока: c = mᵉ mod n
 * 5. Кодирование результата в Base64
 * 
 * @note Для преобразования чисел в байты использует mpz_import/mpz_export
 */
 ...
 * 2. Разбор закрытого ключа
 * 3. Определение размера блока: bitlen(n)/8
 * 4. Разбиение на блоки
 * 5. Дешифрование: m = cᵈ mod n
 * 
 * @warning Требует корректного размера входных данных
 */
