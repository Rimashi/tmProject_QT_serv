...
std::vector<uint8_t> func2client::pad_message(const std::string& message) {
    std::vector<uint8_t> padded(message.begin(), message.end());
    padded.push_back(0x80); // добавляем 1 бит и 7 нулей

    // Добавляем нули, пока длина (в байтах) не станет ≡ 112 mod 128
    while ((padded.size() % 128) != 112) {
        padded.push_back(0x00);
    }

    // Добавляем 16 байт длины сообщения в битах (SHA-384 - 128-битный формат длины)
    // Для SHA-384 старшие 64 бита — нули, младшие — длина в битах
    uint64_t bit_len = (uint64_t)message.size() * 8;

    // Добавляем 8 байт старших 64 бит (все 0 для сообщений < 2^64 бит)
    for (int i = 7; i >= 0; --i) {
        padded.push_back(0x00);
    }

    // Добавляем 8 байт младших 64 бит (длина сообщения в битах, big-endian)
    for (int i = 7; i >= 0; --i) {
...
void func2client::process_block(const uint8_t* block, uint64_t* H) {
    uint64_t W[80];
    for (int t = 0; t < 16; ++t) {
        W[t] = 0;
        for (int i = 0; i < 8; ++i) {
...
std::string func2client::sha384(const std::string& message) {
    uint64_t H[8];
    ...
    // SHA-384 возвращает первые 6 слов (48 байт)
    for (int i = 0; i < 6; ++i) {
        oss << std::hex << std::setw(16) << std::setfill('0') << std::nouppercase << (unsigned long long)H[i];
    }
